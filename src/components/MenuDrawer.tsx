...
export default function MenuDrawer({ open, onClose }: MenuDrawerProps) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed right-0 top-0 bottom-0 w-72 bg-white z-50 shadow-2xl flex flex-col animate-slide-in">
        {/* Header */}
        <div className="bg-magenta text-white px-5 py-5">
        ...
        <nav className="flex-1 py-2 overflow-y-auto">
          <MenuItem href="/" icon="home" label="Startseite" onClick={onClose} />
          <MenuItem href="/new" icon="plus" label="Neue Prüfung" onClick={onClose} />
          <MenuItem href="/entries" icon="list" label="Alle Einträge" onClick={onClose} />
          <MenuItem href="/stats" icon="chart" label="Statistiken" onClick={onClose} />

          <div className="border-t border-gray-100 my-2" />

          <MenuItem href="/settings" icon="settings" label="Einstellungen" onClick={onClose} />
        ...
        <div className="border-t border-gray-100 p-3">
          <form action={logout}>
            <button
              type="submit"
              className="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-danger hover:bg-danger/5 transition"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function MenuItem({ href, icon, label, onClick }: { href: string; icon: string; label: string; onClick: () => void }) {
  ...
  return (
    <Link href={href} onClick={onClick} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition">
      <span className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center text-gray-600">
        {icons[icon]}
      </span>
